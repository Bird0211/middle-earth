...
ace.load_ajax_scripts(scripts, function() {
    ...
    jQuery(function($) {
        ...
        jQuery(grid_selector).jqGrid({
            url: baseurl + "/manage/getAllUsers",
            editurl:baseurl + "/manage/addlg",
            datatype: "json",
            mtype:"post",
            ajaxGridOptions: {
            ...
            cellEdit: false,
            height: 600,
            colNames:['id','UserName','PassWord','Status'],
            colModel:[
                {name:'id',index:'id', width:60, sortable:false, editable:false,hidden:true},
                {name:'username',index:'username', width:60, sortable:false, editable:true},
                {name:'pwd',index:'pwd',width:60, sortable:false, editable:true},
                {name:'status',index:'status',width:60, sortable:false, editable:true}
            ],
            ...
            onCellSelect : function (rowid,iCol,cellcontent,e) {
                ...
                $('#cellinput').bind("blur",function () {
                    ...
                    if(oriVlaue != value){
                        ...
                        if(colModel[iCol].name != 'pwd'){
                            rowData["pwd"] = "";
                        }
                        SendJsonData(baseurl + "/manage/editUser",JSON.stringify(rowData),true,function (data) {
                            console.log(data);
                            toastr.success("设置成功！ ");
...
//		$(grid_selector).jqGrid('filterToolbar',{searchOperators : false});
        ...
        $('#myModal').on('show.bs.modal', function (event) {
            ...
            var body = '<div><table id="TblGrid_grid-table" cellspacing="0" cellpadding="0" border="0">' +
                    '<tbody>' +
                    '<tr rowpos="1" class="FormData" id="tr_name"><td class="CaptionTD">UserName</td><td class="DataTD">&nbsp;' +
                    '<input type="text" id="username" name="value" role="textbox" class="FormElement ui-widget-content ui-corner-all"></td></tr>' +
                    '<tr rowpos="2" class="FormData" id="tr_pwd"><td class="CaptionTD">Password</td><td class="DataTD">&nbsp;' +
                    '<input type="text" id="password" name="value" role="textbox" class="FormElement ui-widget-content ui-corner-all"></td></tr>' +
                    '</tbody></table></div>'
                ;
            ...
            $("#confirm").bind("click",function () {
                ...
                SendJsonData(baseurl + "/manage/adduser",JSON.stringify(data),true,function (data) {
